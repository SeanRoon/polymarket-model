  ...
  *directional*, and the split is now the clearest structure in the whole ledger:
  - **NO-fade half (sell an OVERpriced bin where both sources sit ≥0.10 BELOW the
    ...
    - **AGREEMENT sub-shape — both sources co-locate the truth away IN THE SAME
      ...
      **v15 qualifiers, all required:** (i) tail **≥3 bins from the agreed mode** — measured
      against BOTH sources' modes, and a source whose per-bin value is the Laplace floor
      (0.0093) across the whole event is DEGENERATE and does not count as a vote (see R8/R10);
      (ii) cell has NO large known bias (check `model_bias_applied_f`) and NO negative model
      record — Miami-high-type cells disqualified; (iii′) **downside cap + emptiness test,
      ...
      **(i) and (ii) are the load-bearing qualifiers — do not relax them.**
      **(i) OUT-OF-SAMPLE CONFIRMED (v17, 2026-07-26):** on the JUL27 board I encoded the whole
      v16 chain as a query and ran a drop-one-out sensitivity. **(i) is the ONLY binding
      qualifier** — dropping it alone admits exactly 1 candidate, dropping any other admits 0 —
      and the candidate it blocks is **MIA high B93.5 at 2 bins from the model's mode**, i.e. the
      identical (city, kind, bin) that lost **−$23.77** on JUL25 for exactly that reason, one day
      earlier, at a near-identical price. (i) caught a same-shape repeat of the loss that created
      it. That is the single best piece of evidence in the ledger that these qualifiers are
      correctly calibrated rather than merely restrictive, and it retires the worry that a
      no-trade board means the bar is too high. **Never relax (i) to manufacture a trade.**
