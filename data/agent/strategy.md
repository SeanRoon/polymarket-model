...
- **R2 (weak cells, bar raised in v2):** Cells with a weak, thin, or negative
  ...
  *directional*, and the split is now the clearest structure in the whole ledger:
  - **NO-fade half (sell an OVERpriced bin where both sources sit ≥0.10 BELOW the
    market): 11W–6L, net −$8.11** (v14: +MIA high B93.5 NO @0.78 L −$23.77 — the AGREEMENT
    ...
    - **AGREEMENT sub-shape — both sources co-locate the truth away IN THE SAME
      DIRECTION, so the faded bin is a shared tail: 3W–1L, net −$5.87 (v14, was 3W–0L +$17.90).**
      ...
      **93–94**, the truth in the exact bin they called empty. Two forecasts jointly cold-missed
      ...
      forecast error erases. **This is NO LONGER a proven scaled edge (net-negative on n=4).
      v14 qualifiers, all required:** (i) tail **≥3 bins from the agreed mode**; (ii) cell has
      NO large known bias / negative model record (Miami-high-type cells disqualified);
      (iii) market overpricing in the **0.30–0.45** band (win pays enough to cover the
      full-NO-price downside), NOT the deep ≤0.25 tail. **De-scaled (v14) to 1 cautious
      uncorrelated NO-fade per session** (reverts v7's 2-per-session scale-up).
    ...
    *Kill (agreement subset): it is ALREADY net-negative (−$5.87, 3W–1L) as of v14 — the
    grace it earned as +$17.90 is spent. Kill the subset entirely (stop taking AGREEMENT fades)
    if it reaches losses−wins = +2 (i.e. 3W–5L or worse) OR net −$40, whichever first. Until
    then it survives on de-scaled 1-per-session cautious size under the v14 qualifiers. This
  ...
  *Kill if (whole rule): cumulative R2 record reaches 5 settled losses more than wins
  (v14: R2 is 13W–13L, losses−wins = **0**, wins no longer lead; net −$38.63. The AGREEMENT
  subset that used to carry the edge is now 3W–1L net −$5.87 and de-scaled — R2 is at an
  inflection point, and every remaining sub-shape is at or below break-even. Tighten, don't
  force: no AGREEMENT fade unless it clears all three v14 qualifiers.).*
