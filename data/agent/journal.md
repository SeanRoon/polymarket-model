...
`2026-08-04/1130.parquet`, md5 `24009392…` — byte-identical to the file I swept 13 minutes ago — so **R20's
byte-identical fast path applies** and under R20(c) no bin can newly enter the candidate set on a live price
move alone. The 12:02 sweep of this exact file already adjudicated it: `--min-lead-hours 18` returned
`_none at this threshold_` because AUG04 is the only listed board and its bins are 5–8h from close, failing
R12′'s ≥18h clause; the 14 largest raw gaps were all degenerate model_p (0.01/0.95) with NBM disagreeing —
the stale-model signature, not an edge. No `agent-scan` census: the AUG5 listing window is 14:00–15:10 UTC
and it is 12:15, so a census now cannot move a rule.

Strategy stays **v37** — nothing settled, nothing graded, no rule touched.
*Cadence: 13 min after the 12:02 session. `1130` landed at 11:34 after a 140-min overnight gap; if the
daytime cadence tightens as it did yesterday the next file is due ~12:35–13:00. The first session that can
carry a trade is still the one after ~15:15 UTC, once AUG5 has both listed and been covered by a snapshot.*
...
`2026-08-04/0910.parquet`, md5 `6074c1ad…` — identical across the 09:15 / 09:19 / 10:05 / 10:15 / 10:19 /
11:02 / 11:15 / 11:19 reads, now 128 min old — so **R20's fast path applies** and under R20(c) no bin can
newly enter the candidate set on a live price move alone. `agent-model-view --min-lead-hours 18` returns
`_none at this threshold_` unchanged: AUG04 is still the only listed board and every bin is now 5–8h from
close, failing R12′'s ≥18h clause before any edge or book test runs. No `agent-scan` census — the AUG5
listing window is 14:00–15:10 UTC and it is 11:19, so a census now cannot move a rule.

Strategy stays **v37** — nothing settled, nothing graded, no rule touched.
*Cadence: 4 min after the 11:15 session, a sixth sub-hour re-fire in this stretch and the tightest yet — the
inputs could not have moved in the interval and did not. The `0910` file is 128 min old against the confirmed
~3h overnight recorder gap (0315 → 0610 → 0910), so the next snapshot is due ~12:10 ± 30 min. The first
session that can carry a trade is still the one after ~15:15 UTC, once AUG5 has both listed and been covered
by a snapshot.*

## 2026-08-04 11:15 UTC — 13-min re-fire; `0910` unchanged for a sixth read; 0 trades

11:15 UTC — nothing settled (`settled=0 still_open=1`), no qualifying edge, holding 1 position (SEA high
B88.5 NO ×20, $16.04 at risk; $853.17 free cash, 41 settled at 20W–21L, −$130.79). Snapshot is **still**
`2026-08-04/0910.parquet`, md5 `6074C1AD…` — identical across the 09:15 / 09:19 / 10:05 / 10:15 / 10:19 /
11:02 / 11:15 reads, now 124 min old — so **R20's fast path applies** and under R20(c) no bin can newly
enter the candidate set on a live price move alone. `agent-model-view --min-lead-hours 18` returns
`_none at this threshold_` unchanged: AUG04 is still the only listed board and every bin is now 5–8h from
close, failing R12′'s ≥18h clause before any edge or book test runs. No `agent-scan` census — the AUG5
listing window is 14:00–15:10 UTC and it is 11:15, so a census now cannot move a rule.

Strategy stays **v37** — nothing settled, nothing graded, no rule touched.
*Cadence: 13 min after the 11:02 session, a fifth sub-hour re-fire in this stretch. The `0910` file is now
124 min old against the confirmed ~3h overnight recorder gap (0315 → 0610 → 0910), so the next snapshot is
due ~12:10 ± 30 min. The first session that can carry a trade is still the one after ~15:15 UTC, once AUG5
has both listed and been covered by a snapshot.*

## 2026-08-04 11:02 UTC — hourly slot; `0910` unchanged for a fifth read; 0 trades

11:02 UTC — nothing settled (`settled=0 still_open=1`), no qualifying edge, holding 1 position (SEA high
B88.5 NO ×20, $16.04 at risk; $853.17 free cash, 41 settled at 20W–21L, −$130.79). Snapshot is **still**
`2026-08-04/0910.parquet`, md5 `6074c1ad…` — identical across the 09:15 / 09:19 / 10:05 / 10:15 / 10:19 /
11:02 reads, now 112 min old — so **R20's fast path applies** and under R20(c) no bin can newly enter the
candidate set on a live price move alone. `agent-model-view --min-lead-hours 18` returns `_none at this
threshold_` unchanged: AUG04 is still the only listed board and every bin is now 5–8h from close, failing
R12′'s ≥18h clause before any edge or book test runs. No `agent-scan` census — the AUG5 listing window is
14:00–15:10 UTC and it is 11:02, so a census now cannot move a rule.

Strategy stays **v37** — nothing settled, nothing graded, no rule touched.
*Cadence: 43 min after the 10:19 re-fire, back on the normal hourly slot after four sub-hour re-fires. The
`0910` file is 112 min old against the confirmed ~3h overnight recorder gap (0315 → 0610 → 0910), so the
next snapshot is due ~12:10 ± 30 min. The first session that can carry a trade is still the one after
~15:15 UTC, once AUG5 has both listed and been covered by a snapshot.*
...
B88.5 NO ×20, $16.04 at risk; $853.17 free cash, 41 settled at 20W–21L, −$130.79). Snapshot is **still**
`2026-08-04/0910.parquet`, md5 `6074c1ad…` — byte-identical to the 09:15 / 09:19 / 10:05 reads, now 64 min
old — so **R20's fast path applies** and under R20(c) no bin can newly enter the candidate set on a live
price move alone. `agent-model-view --min-lead-hours 18` returns `_none at this threshold_` unchanged:
AUG04 is still the only listed board and every bin is 6–9h from close, failing R12′'s ≥18h clause before
any edge or book test runs. No `agent-scan` census — this fired 10 minutes after the last session, against
an unchanged file, and the AUG5 listing window is 14:00–15:10 UTC.

Strategy stays **v37** — nothing settled, nothing graded, no rule touched.
*Cadence: 10 min after the 10:05 session, the third sub-hour re-fire in this stretch. Next snapshot due
~12:10 ± 30 min against the confirmed ~3h overnight recorder gap. The first session that can carry a trade
is still the one after ~15:15 UTC, once AUG5 has both listed and been covered by a snapshot.*

## 2026-08-04 10:05 UTC — back on the hourly slot; R20 fast path on `0910` again; 0 trades

10:05 UTC — nothing settled (`settled=0 still_open=1`), no qualifying edge, holding 1 position (SEA high
B88.5 NO ×20, $16.04 at risk; $853.17 free cash, 41 settled at 20W–21L, −$130.79). Snapshot is **still**
`2026-08-04/0910.parquet`, md5 `6074c1ad…` — byte-identical to what the 09:15 and 09:19 sessions read, now
54 min old — so **R20's fast path applies** and under R20(c) no bin can newly enter the candidate set on a
live price move alone. `agent-model-view --min-lead-hours 18` still returns `_none at this threshold_`:
AUG04 remains the only listed board and every bin is 6–9h from close, failing R12′'s ≥18h clause before any
edge or book test runs. No `agent-scan` census this hour — the AUG5 listing window is 14:00–15:10 UTC and
it is 10:05, so a census now cannot move a rule.

Strategy stays **v37** — nothing settled, nothing graded, no rule touched.
*Cadence: 46 min after the 09:19 re-fire, back on the normal hourly slot after two sub-5-minute re-fires.
The `0910` file is 54 min old against the confirmed ~3h overnight recorder gap, so the next snapshot is due
~12:10 ± 30 min. The first session that can carry a trade is still the one after ~15:15 UTC, once AUG5 has
both listed and been covered by a snapshot.*

## 2026-08-04 09:19 UTC — off-cadence re-fire 4 min later; R20 fast path on `0910`; 0 trades

09:19 UTC — nothing settled (`settled=0 still_open=1`), no qualifying edge, holding 1 position (SEA high
B88.5 NO ×20, $16.04 at risk; $853.17 free cash, 41 settled at 20W–21L, −$130.79). Snapshot is still
`2026-08-04/0910.parquet` md5 `6074c1ad…` — the same file the 09:15 session re-derived its refusal from,
now 8 min old — so **R20's byte-identical fast path applies** and, under R20(c), no bin can newly enter the
candidate set on a live price move alone. `agent-model-view --min-lead-hours 18` returns
`_none at this threshold_` unchanged: **AUG04 is still the sole listed board and every bin is 7–10h from
close**, so the whole board fails R12′'s ≥18h clause before any edge or book test runs.

I did **not** re-run `agent-scan` for a board census this hour. The AUG5 listing window is measured at
14:00–15:10 UTC and it is 09:19; a census taken 5 minutes after the last one, against an unchanged snapshot,
cannot move any rule. Stating that explicitly rather than implying a check I didn't make.

Strategy stays **v37** — nothing settled, so per the editing rules I graded nothing and touched no rule.
*Cadence: 4 min after the 09:15 session, the second sub-5-minute re-fire in this stretch. The `0910` file
arrived on schedule against the confirmed ~3h overnight recorder gap, so the next is due ~12:10 ± 30 min.
The first session that can carry a trade is still the one after ~15:15 UTC, once the AUG5 board has both
listed and been covered by a snapshot.*
...
B88.5 NO ×20, $16.04 at risk; $853.17 free cash, 41 settled at 20W–21L, −$130.79). Snapshot is still
`2026-08-04/0610.parquet` md5 `3a15d664…` — **ninth** session on it, now **171 min** old — so **R20's
byte-identical fast path holds** and R20(c) bars any entrant on a live move alone. R12′ re-tested anyway and
refuses unchanged: `agent-model-view --min-lead-hours 18` returns `_none at this threshold_`, AUG04 still the
sole listed board and deep inside the ≥18h exclusion, AUG5 unlisted until its 14:00–15:10 UTC window (~5h
out). Strategy stays **v37**, nothing graded, no rule touched. *Cadence: 44 min after the 08:18 re-fire, back
on the :02 hourly beat after seven sub-hour fires overnight. The 171-min age now matches the measured
`0315`→`0610` overnight gap of 175 min almost exactly, so the next file is due within the hour — the ~09:05
± 30 min estimate from 08:02 stands and is about to be tested. The first session that can carry a trade is
still the one after ~15:15 UTC, once the AUG5 board has both listed and been covered by a snapshot.*

## 2026-08-04 08:18 UTC — off-cadence re-fire 3 min later; eighth session on 0610; 0 trades

08:18 UTC — nothing settled (`settled=0 still_open=1`), no qualifying edge, holding 1 position (SEA high
B88.5 NO ×20, $16.04 at risk; $853.17 free cash, 41 settled at 20W–21L, −$130.79). Snapshot is still
`2026-08-04/0610.parquet` md5 `3a15d664…` — **eighth** session on it, now **128 min** old — so **R20's
byte-identical fast path holds** and R20(c) bars any entrant on a live move alone. R12′ re-tested anyway and
refuses unchanged: `agent-model-view --min-lead-hours 18` returns `_none at this threshold_`, AUG04 still the
sole listed board and deep inside the ≥18h exclusion, AUG5 unlisted until its 14:00–15:10 UTC window (~6h
out). Strategy stays **v37**, nothing graded, no rule touched. *Cadence: 3 min after the 08:15 session — the
seventh sub-hour re-fire since midnight and the shortest yet; a no-op by construction, since neither the
snapshot nor the board turns over on a 3-minute timescale. The overnight band's early edge stays ruled out,
so the next file is still expected ~09:05 ± 30 min; the first session that can carry a trade remains the one
after ~15:15 UTC, once the AUG5 board has both listed and been covered by a snapshot.*

## 2026-08-04 08:15 UTC — off-cadence re-fire 13 min later; seventh session on 0610; 0 trades

08:15 UTC — nothing settled (`settled=0 still_open=1`), no qualifying edge, holding 1 position (SEA high
B88.5 NO ×20, $16.04 at risk; $853.17 free cash, 41 settled at 20W–21L, −$130.79). Snapshot is still
`2026-08-04/0610.parquet` md5 `3a15d664…` — **seventh** session on it, now **125 min** old — so **R20's
byte-identical fast path holds** and R20(c) bars any entrant on a live move alone. R12′ re-tested anyway and
refuses unchanged: `agent-model-view --min-lead-hours 18` returns `_none at this threshold_`, AUG04 still the
sole listed board and deep inside the ≥18h exclusion, AUG5 unlisted until its 14:00–15:10 UTC window (~6h
out). Strategy stays **v37**, nothing graded, no rule touched. *Cadence: 13 min after the 08:02 session — the
sixth sub-hour re-fire since midnight, and a no-op by construction (neither the snapshot nor the board turns
over on a 13-minute timescale). The overnight band's early edge is already ruled out (see 08:02), so the next
file is still expected ~09:05 ± 30 min; the first session that can carry a trade remains the one after
~15:15 UTC, once the AUG5 board has both listed and been covered by a snapshot.*

08:02 UTC — nothing settled (`settled=0 still_open=1`), no qualifying edge, holding 1 position (SEA high
B88.5 NO ×20, $16.04 at risk; $853.17 free cash, 41 settled at 20W–21L, −$130.79). Snapshot is still
`2026-08-04/0610.parquet` md5 `3a15d664…` — **sixth** session on it, now **112 min** old — so **R20's
byte-identical fast path holds** and R20(c) bars any entrant on a live move alone. R12′ re-tested anyway
and refuses unchanged: `agent-model-view --min-lead-hours 18` returns `_none at this threshold_`, AUG04
still the sole listed board and deep inside the ≥18h exclusion, AUG5 unlisted until its 14:00–15:10 UTC
window (~6h out). Strategy stays **v37**, nothing graded, no rule touched. *Cadence: 42 min after the 07:20
re-fire, back on the :02 hourly beat after four sub-hour fires overnight. On the revised overnight band
(`0315`→`0610` measured 175 min) the next file was due ~08:00–10:00 UTC and has not landed at 08:02 — the
band's early edge is now ruled out, so expect it ~09:05 ± 30 min. The first session that can carry a trade
is still the one after ~15:15 UTC, when the AUG5 board has both listed and been covered by a snapshot.*

## 2026-08-04 07:20 UTC — off-cadence re-fire 4 min later; fifth session on 0610; 0 trades

07:20 UTC — nothing settled (`settled=0 still_open=1`), no qualifying edge, holding 1 position (SEA high
B88.5 NO ×20, $16.04 at risk; $853.17 free cash, 41 settled at 20W–21L, −$130.79). Snapshot is still
`2026-08-04/0610.parquet` md5 `3a15d664…`, commit 06:14:27Z — **fifth** session on it, 68 min old — so
**R20's byte-identical fast path holds** and R20(c) bars any entrant on a live move alone. R12′ re-tested
anyway and refuses unchanged: `agent-model-view --min-lead-hours 18` returns `_none at this threshold_`,
AUG04 still the sole listed board and deep inside the ≥18h exclusion, AUG5 unlisted until its 14:00–15:10
UTC window (~7h out). Strategy stays **v37**, nothing graded, no rule touched. *Cadence: 4 min after the
07:16 session — the fifth sub-hour re-fire of the night and, like the others, a no-op by construction:
neither the snapshot nor the board can turn over on a 4-minute timescale. On the revised overnight band
(`0315`→`0610` measured 175 min) the next file is due ~08:00–10:00 UTC; the first session that can carry a
trade is still the one after ~15:15 UTC.*

## 2026-08-04 07:16 UTC — off-cadence re-fire 14 min later; fourth session on 0610; 0 trades

07:16 UTC — nothing settled (`settled=0 still_open=1`), no qualifying edge, holding 1 position (SEA high
B88.5 NO ×20, $16.04 at risk; $853.17 free cash, 41 settled at 20W–21L, −$130.79). Snapshot is still
`2026-08-04/0610.parquet` md5 `3A15D664…` (fourth session on it, 66 min old), so **R20's byte-identical
fast path holds** and R20(c) bars any entrant on a live move alone. R12′ re-tested anyway and refuses
unchanged: `agent-model-view --min-lead-hours 18` returns `_none at this threshold_`, AUG04 still the sole
listed board and deep inside the ≥18h exclusion, with the AUG5 board unlisted until its 14:00–15:10 UTC
window (~7h out). Strategy stays **v37**, nothing graded, no rule touched. *Cadence: 14 min after the 07:02
session — a fourth sub-hour re-fire, and by construction a no-op since neither the snapshot nor the board
can turn over on that timescale. Next file due ~08:00–10:00 UTC on the revised overnight band; the first
session that can carry a trade is still the one after ~15:15 UTC.*

## 2026-08-04 07:02 UTC — back on cadence; third session on 0610; 0 trades

07:02 UTC — nothing settled (`settled=0 still_open=1`), no qualifying edge, holding 1 position (SEA high
B88.5 NO ×20, $16.04 at risk; $853.17 free cash, 41 settled at 20W–21L, −$130.79). Snapshot is still
`2026-08-04/0610.parquet` md5 `3A15D664…` (third session on it, 51 min old), so **R20's byte-identical fast
path holds** and R20(c) bars any entrant on a live move alone. R12′ re-tested anyway and refuses unchanged:
`agent-model-view --min-lead-hours 18` returns `_none at this threshold_`, AUG04 still the sole listed board
and deep inside the ≥18h exclusion, with the AUG5 board unlisted until its 14:00–15:10 UTC window (~7–8h out).
Strategy stays **v37**, nothing graded, no rule touched. *Cadence: 43 min after the 06:19 re-fire, back on the
:02 beat. On last session's revised overnight band the next file is due ~08:00–10:00 UTC; the first session
that can carry a trade is still the one after ~15:15 UTC, when the AUG5 board has both listed and been covered
by a snapshot.*

## 2026-08-04 06:19 UTC — off-cadence re-fire 3 min later; second session on 0610; 0 trades

06:19 UTC — nothing settled (`settled=0 still_open=1`), no qualifying edge, holding 1 position (SEA high
B88.5 NO ×20, $16.04 at risk; $853.17 free cash, 41 settled at 20W–21L, −$130.79). Snapshot is still
`2026-08-04/0610.parquet` — the same file the 06:16 session swept **3 minutes** ago — so **R20's
byte-identical fast path holds** and R20(c) bars any entrant on a live move alone. R12′ re-tested anyway
and refuses unchanged: `agent-model-view --min-lead-hours 18` returns `_none at this threshold_`, AUG04
being the sole listed board at ~11.5–14.5h to close and the AUG5 board unlisted until its 14:00–15:10 UTC
window (~8h out). Strategy stays **v37**, nothing graded, no rule touched. *Cadence: the third
sub-5-minute re-fire pattern of the night — nothing in the input set can move on that timescale, so this is
a no-op by construction. Next snapshot due ~08:00–10:00 UTC on the revised overnight band; the first
session that can carry a trade is still the one after ~15:15 UTC, when the AUG5 board has both listed and
been covered by a snapshot.*
...
B88.5 NO ×20, $16.04 at risk; $853.17 free cash, 41 settled at 20W–21L, −$130.79); snapshot still
`2026-08-04/0315.parquet` md5 `41eebc0b…` (sixth consecutive session, ~123 min old, commit 03:24:00Z), so
**R20's byte-identical fast path holds** and R20(c) bars any entrant on a live move alone. R12′'s refusal
carries unchanged — AUG04 is still the sole listed board and sits inside the ≥18h exclusion; AUG5 board
unlisted (window 14:00–15:10 UTC). Strategy stays **v37**, nothing graded, no rule touched. *This fired
**3 minutes** after the 05:15 session, which itself fired 13 min after 05:02 — three invocations inside 16
minutes against a ~205 min overnight snapshot cadence. Nothing in the input set can move on that timescale,
so all three are no-ops by construction; the clustering is the harness, not the market. Next snapshot due
~06:45–07:15 UTC — that is the first session that can carry information.*

## 2026-08-04 05:15 UTC — off-cadence re-fire 13 min later; fifth session on 0315; 0 trades

05:15 UTC — nothing settled (`settled=0 still_open=1`), no qualifying edge, holding 1 position (SEA high
B88.5 NO ×20, $16.04 at risk; $853.17 free cash, 41 settled at 20W–21L, −$130.79); snapshot still
`2026-08-04/0315.parquet` md5 `41eebc0b…` (fifth consecutive session, ~112 min old, commit 03:24:00Z), so
**R20's byte-identical fast path holds** and R20(c) bars any entrant on a live move alone. R12′'s refusal
carries unchanged — AUG04 is the sole listed board and is now inside the ≥18h exclusion; AUG5 board still
unlisted (window 14:00–15:10 UTC). Strategy stays **v37**, nothing graded, no rule touched. *This fired 13
min after the 05:02 session, off the hourly beat; no input can move in 13 min against a ~205 min overnight
snapshot cadence, so it is a no-op by construction. Next snapshot due ~06:45–07:15 UTC.*
...
B88.5 NO ×20, $16.04 at risk; $853.17 free cash, 41 settled at 20W–21L, −$130.79); snapshot still
`2026-08-04/0315.parquet` md5 `41eebc0b…` (third consecutive session on it), so **R20's byte-identical fast
path holds** and R20(c) still bars any entrant on a live move alone. R12′'s refusal likewise carries: the
sole listed board is AUG04 at ~12.7–15.7h to close, under the ≥18h clause; AUG5 board still unlisted (window
14:00–15:10 UTC). Strategy stays **v37**, nothing graded, no rule touched. *Note: this fired **4 minutes**
after the 04:15 session, not on the hourly beat — an off-cadence invocation, not a state change. No inputs
can move in 4 min (snapshot cadence is ~200+ min overnight), so the session is a pure no-op by construction.
Worth remembering when reading the log: two adjacent entries minutes apart are the harness, not the market.*
...
B88.5 NO ×20, $16.04 at risk; $853.17 free cash, 41 settled at 20W–21L, −$130.79); snapshot still
`2026-08-03/2350.parquet` md5 `85f25f7c…` (twelfth consecutive session; committed 23:55:59Z, ~202 min old,
now at the low edge of the measured 221–231 min overnight band), so R20's byte-identical fast path holds and
R20(c) bars any new entrant on a live move alone — same empty qualifying set; strategy stays **v37**, nothing
graded, no rule touched; AUG5 board not listed yet (window 14:00–15:10 UTC). *One line, per the standing
hygiene note; the v38 journal-collapse rule still waits for a real-work session.*

## 2026-08-04 03:16 UTC — fast path, eleventh session on the same snapshot, 0 trades

03:16 UTC — nothing settled (`settled=0 still_open=1`), no qualifying edge, holding 1 position (SEA high
B88.5 NO ×20, $16.04 at risk; $853.17 free cash, 41 settled at 20W–21L, −$130.79); snapshot still
`2026-08-03/2350.parquet` md5 `85f25f7c…` (eleventh consecutive session; committed 23:55:59Z, ~200 min old,
inside the measured 221–231 min overnight band), so R20's byte-identical fast path holds and R20(c) bars any
new entrant on a live move alone — same empty qualifying set; strategy stays **v37**, nothing graded, no rule
touched; AUG5 board not listed yet (window 14:00–15:10 UTC). *One line, per the standing hygiene note; the
v38 journal-collapse rule still waits for a real-work session.*
...
B88.5 NO ×20, $16.04 at risk; $853.17 free cash, 41 settled at 20W–21L, −$130.79). Newest snapshot is
still `2026-08-03/2350.parquet`, md5 `85f25f7c…` — byte-identical for the **eighth** consecutive session.
R19′ disclosure: ~144 min past its 23:51:56Z run start, against measured overnight gaps of 225/221/231 min
— still inside a normal overnight window, not late. R20's byte-identical fast path applies and R20(c) bars
any bin from newly entering the candidate set on a live-price move alone, so the qualifying set is the same
empty set the 00:10 full sweep produced. Strategy stays **v37**; nothing settled, so nothing graded and no
rule touched. AUG5 weather still unlisted (measured listing window 14:00–15:10 UTC). Next session with real
work: the first after a genuinely new snapshot, or after 14:00 UTC when the AUG5 board should list.
...
`agent-settle settled=0 still_open=1`; **$853.17** free cash, 41 settled at **20W–21L, −$130.79**. Strategy
stays **v36** (nothing settled ⇒ nothing graded, no rule touched). Newest snapshot is still `1825.parquet`
(95 min old) — third consecutive session on it, so R22(a)'s candidate set is byte-identical and under
R20(c) nothing can newly enter via a live move. **AUG5 still not listed** (`grep -c 26AUG05` over the
live weather scan = **0**), so no new board and nothing new to adjudicate; the two LAX names were re-quoted
45 minutes ago and both fail (iii‴)'s 0.85 entry cap by ≥0.02, which no plausible hour-scale move closes.
**Mark on SEA high AUG4 B88.5 NO ×20 @ 0.79:** live yes **0.15 / 0.16**, mid **0.155** — unchanged from
19:18, ~**+$0.86** unrealized, my way. Bookkeeping unchanged: R23 sole-block **1**, R20(c) sole-block **1**,
swept boards **4**, composition-risk count **1 board / 1 trade**. **Want by next session:** the AUG5 listing
(R12 window ~14:00–15:10 UTC ⇒ tomorrow) and KLAX's AUG4 low vs the 69–70°F pre-registration that scores
R20(c)'s only sole block.

## 2026-08-03 19:18 UTC — nothing settled, no qualifying edge, holding 1 position.

`agent-settle settled=0 still_open=1`; **$853.17** free cash, 41 settled at **20W–21L, −$130.79**. Strategy
stays **v36** (nothing settled ⇒ nothing graded, no rule touched). Snapshot is still `1825.parquet` — the
same file swept 14 minutes ago, 50 min old — so R22(a)'s candidate set is byte-identical and under R20(c)
nothing can newly enter via a live move; the only possible change was a drop-out or a new board. **AUG5
still not listed** (`KXHIGHNY`/`KXHIGHLAX`/`KXHIGHTSEA` open events: AUG3, AUG4 only), so no new board.
Re-quoted the two adjudicable names at 19:17: **LAX high B79.5** yes **0.11 / 0.13** (mid 0.12 ⇒ NO entry
**0.87**, still above (iii‴)'s 0.85 cap — the ask widened a cent, moving it *away* from qualifying);
**LAX low B69.5** unchanged in substance from 19:03, still failing both (iii‴)'s cap and R2's 0.15 edge bar.
Fourteen minutes produced no new information, exactly as last session's 105-minute measurement predicted for
a mature board. **Mark on SEA high AUG4 B88.5 NO ×20 @ 0.79:** live yes **0.15 / 0.16**, mid 0.155 (from
0.165 at 19:03) — ~**+$0.86** unrealized, my way. Bookkeeping unchanged: R23 sole-block **1**, R20(c)
sole-block **1**, swept boards **4**, composition-risk count **1 board / 1 trade**. **Want by next session:**
the AUG5 listing (R12 window ~14:00–15:10 UTC, so tomorrow) and KLAX's AUG4 low vs the 69–70°F
pre-registration that scores R20(c)'s sole block.
...
file the 06:20 and 07:15 sessions used; I re-checked mtime **after** the pull per the 06:20 addendum and
nothing new came down. **Nothing settled, so no grading step is owed** — but the version moved anyway,
because what I learned this hour was about my own procedure, not about an outcome.

**Why I did not take the fast path.** R20's premise is that a byte-identical snapshot yields the same
survivor set by construction. That is true — but it makes the fast path only as good as the sweep it
inherits from, and this hour I decided to check one loose end (a Dallas high bin that looked like a
candidate and appeared in no prior adjudication) rather than assume. Checking it meant writing the funnel as
SQL, and the SQL disagreed with my own prose.

**R22 — the finding, stated plainly because it is embarrassing.** The 06:20 addendum called itself a full
re-sweep of `0610.parquet` and reported one high-side survivor (MIA B94.5), with "everything else" being a
blacked-out low, an R21 cell, or an already-modal bin. Re-run as a query against the identical file, **four
qualifying bins on that board are none of those three things**:

| candidate | shape | model_p | nbm_p | mid | R18 ratio | why missed |
|:---|:---|--:|--:|--:|--:|:---|
| DAL high B102.5 | **YES-buy** | 0.806 | 0.427 | 0.295 | — | wrong direction — never screened |
| LV high B111.5 (JUL28) | NO-fade | 0.009 | 0.029 | 0.290 | 0.423 | below where I stopped reading |
| PHIL high B81.5 | NO-fade | 0.083 | 0.095 | 0.325 | 0.890 | below where I stopped reading |
| OKC high B102.5 | NO-fade | 0.139 | 0.224 | 0.370 | 0.961 | below where I stopped reading |

**The mechanism is R13′ pointed back at me.** R13′ proved that a bin's both-sources-below gap is bounded
above by the price the market put there, so the biggest gaps only exist where the market put its mass — the
**top of an edge-sorted view is structurally the modal bins**, which R5a bans universally. Reading the top
rows and stopping therefore samples the funnel in precisely the region where a candidate cannot be. R13′ has
been telling me since v17 to hunt the 2nd/3rd-priced bins. I wrote that down five times and kept reading
row 1. **And the direction half is worse than the truncation half:** `agent-model-view` sorts on *signed*
edge, so YES-side and NO-side candidates sit at opposite ends of one table — **I have not enumerated the YES
direction in weeks.** R2's YES-buy half is 2W–7L, −$30.52, one settlement from its own pre-registered
restriction; a losing sub-rule I never screen for can never reach its kill clause, and its silence in my
journals was quietly reading as "that shape doesn't occur."

**All four refused, every one on a rule that already existed. Nothing was loosened to fit them.**

- **DAL high B102.5, YES @0.30 → R5(b).** This is v23's DAL T101 shape verbatim. Nine committed cycles:
  B102.5 mid **0.375 → 0.445 → 0.305 → 0.320 → 0.295 → 0.255 → 0.255 → 0.275 → 0.295** while the market's
  mode B100.5 ran **0.285 → 0.345 → 0.395 → 0.445 → 0.495 → 0.510 → 0.485 → 0.515 → 0.545**. Over ~15h the
  market moved **+0.26** into the bin my sources reject and **−0.15** out of the bin they like. Model went
  0.25 → 0.71 → 0.82 → 0.806 the other way. **The edge grew 0.27 → 0.51 and about half that growth is pure
  adverse repricing** — R5(b)'s definition of a trade not to take. NBM corroborates the market rather than
  me: q50 **102.50 → 102.46 → 102.35**, cooler each cycle, *following* the tape — the same detail that
  strengthened the T101 refusal. (It would also have failed a mirrored (ii‴): Dallas/high NBM runs **+1.44°F
  warm, 4 of 5**, and correcting 102.35 − 1.44 = **100.91** lands in the market's modal bin, out of the bin I
  would be buying. I did **not** promote that mirror to a rule — R5(b) already refuses it, and inventing a
  gate to re-kill something already dead is the R16 pattern.)
- **LV high B111.5 JUL28 → (ii‴), re-confirmed on a fresh cycle.** Its own founding case, and it still
  fires: LV/high NBM cold **5 of 5** (−1.77, −2.29, −3.73, −0.97, −2.91; mean **−2.33°F**), today's q50
  **108.56 + 2.33 = 110.89**, inside the faded 111–112 bin's [110.5, 112.5) support, and **JUL26 realized
  111** — in that exact bin. Everything else about it was clean: non-modal, R18 ratio 0.423 (mid-support),
  both sources ≤0.05 so (iii′)'s emptiness test passes, NO entry 0.72 ≤ 0.85, 2¢ spread. That is exactly the
  candidate (ii‴) was written for.
- **PHIL high B81.5 → BRACKET *and* (ii‴).** The sources are not agreeing at all: model's mode is **T84
  @0.454** (≥85°F), NBM's is **T77 @0.578** (≤76°F) — 8°F apart, rejecting the 81–82 bin from **opposite
  sides**, market sitting between them. That is the SFO low B61.5 shape (0W–1L, −$28.59): fading a bracket
  shoulder fades forecast disagreement, and the truth lands there disproportionately. Independently,
  PHIL/high NBM runs **−2.20°F cold 5/5**, so correcting 76.28 → 78.48 moves toward the bin I would sell.
  Also: q50 **76.28** against a market mode of 83–84 at `nbm_lead_hours` **27** is the **v24 Northeast
  stale-18Z cold displacement**, a documented artifact, not an edge.
- **OKC high B102.5 → BRACKET + R18 at the worst ratio I have screened.** Model's mode **B104.5 @0.750**,
  NBM's **B100.5 @0.457**, faded bin the shoulder between. Market: B104.5 @0.385 vs B102.5 @0.370 →
  **R18 ratio 0.961**, outside my 0.33–0.76 support and worse than the 0.948 R18 was written to refuse. A
  genuine two-way coin flip, not an overpriced tail. Worth recording that **(ii‴) does not fire here** —
  OKC/high NBM error is +2.10, +2.24, −0.31, −1.84, −1.60, **mean +0.12, signs mixed 3/2** — so the rule is
  discriminating between cells rather than blanket-vetoing.

**(ii‴)'s firing ratio, re-checked as v31 promised, and the counting fixed.** It fired on **3 of 4** again,
and v31 had flagged 3-of-4 as approaching "eating the funnel" — I was one session from narrowing it. But MIA
is independently killed by (ii′) and PHIL is independently a BRACKET: **(ii‴) is the sole blocker on 1 of
4.** Adopted as a convention for every anti-learning-blocker tripwire I own: **count sole-blocker firings,
not firings.** A veto that co-fires with an independent kill removes nothing I would have taken. This is how
R5(b)'s log was always kept; v32 generalizes it rather than inventing it.

**Correction to my own reading of R19′.** The 04:15, 05:15, 06:15 and 07:15 sessions all cited "R19′'s
60–110 min overnight baseline." **That is the 11:00–23:45 UTC row.** The table reads 01:00→05:00 =
3h20m–4h05m, 05:00→10:00 = 2h10m–3h35m. So the 06:15 disclosure ("156 min, outside the baseline") was owed
under a misread, and today's 121-min snapshot at 08:15 UTC is *fresher* than its 2h10m–3h35m baseline — **no
disclosure owed.** No refusal in those sessions rested on it, but R19′ exists precisely to stop me quoting a
staleness premise I have not checked, and I reintroduced the error by quoting the wrong row from memory.

**Board coverage for completeness.** All 20 **low** events are inside R12″'s local-midnight-to-10:00
blackout (08:15 UTC = 04:15 EDT / 03:15 CDT / 02:15 MDT / 01:15 PDT, and earlier still in LST) — unscreenable,
third consecutive session. Highs are screenable under R12′ (all cities well before 09:00 local, lead 10–13h).
AUS/SATX/DEN highs remain **closed under R21**; the eye-catching SATX T97 at model_p 0.95 vs mid 0.27 is that
rule's degenerate column, not an edge. JUL29 still has no source coverage (**R12‴**, fifth session).

**Position.** LV JUL27 B111.5 NO (30 @ 0.70, $21.45 at risk) — the market is **`status=closed`** as of 08:00
UTC with **`result` empty** and the book gone. Marks ~+$8.55 on the last two-sided quotes. **Closed is not
settled**: `agent-settle` returned `settled=0` and it decides, not the tape or the clock.

**Trades opened: none.** Holding 1. Strategy **v31 → v32** (R22 added; (ii‴) tripwire counting convention;
R19′ misreading corrected).

**What I want by next session:** the LV settlement — it is the first graded trade since v27, the first
out-of-sample test of the clean non-modal AGREEMENT fade under the v18 qualifiers, and it should finally
write `resolutions.parquet` so R21's stamp test can run. And the first R22 query on a genuinely new snapshot,
to see whether the two-direction enumeration keeps finding things the sorted view hides.
