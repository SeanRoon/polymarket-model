...
uv run polymarket fetch-climatology          # refresh ERA5 day-of-year climatology (rare; ~once a year)
...
- **Phase 3 (in progress since 2026-07-16):** EMOS / NGR is live as a recorded-only pilot (`calibration/emos.py`): per-(station, kind) Gaussians fit by CRPS minimization over the snapshot+resolution history, written to `data/station_emos.parquet` by `compute-emos` (daily in `resolve.yml`), applied by the recorder as `emos_p`/`emos_mu_f`/`emos_sigma_f` columns. **Fits are in ANOMALY space** (since 2026-07-17): both regression sides are deviations from the station's ERA5-derived day-of-year climatology (`weather/climatology.py`, `data/station_climatology.parquet`, refreshed rarely via `fetch-climatology` — the write merges per-station so partial refreshes are safe), i.e. `mu = climo + a + b·(ens_mean − climo)`, `sigma = sqrt(c + d·ens_var)`. This keeps coefficients season-independent — without it the intercept memorizes the current season's mean (the first raw-space KLAX fit anchored to ~70°F, which is July, not LA) and every seasonal transition forces the rolling 60-day window to re-learn it. The emos parquet carries a `feature_space` tag; `load_emos` ignores rows from a different space, and events with no climatology get no `emos_p`. Scope is `settings.emos_stations` — the veteran excluded stations KLAX/KMDW/KMIA/KNYC only; **never a station with a live cell (KAUS/KDEN/KSAT), operator mandate 2026-07-16**. Because `a`/`b` absorb the mean error, EMOS subsumes the rolling bias correction: the fit reconstructs the RAW pre-biascorr ensemble mean (adding `model_bias_applied_f` back) and the apply path does the same, so it never stacks on top of biascorr. Train/apply features are both reconstructed from bin probs via bin centers (`moments_from_bin_probs` mirrors the fit SQL — keep them in lockstep). Graduation bar: forward-recorded `emos_p` Brier beats the market midpoint out-of-sample, same as KSAT/high — `compare-to-resolved` scores it (`brier_emos_mean` vs the same-row `brier_model_on_emos`/`brier_market_on_emos`) and `diagnose` emits `emos_beats_market` (good) / `emos_improves_model` (info) / `emos_underperforms` (warn) flags the model-watch agent reads. Still to come: isotonic layered on EMOS residuals, KDE+climatology, and the June-18 cohort once it has ~45 days of pairs.
...
- `polymarket agent-trade TICKER --side yes|no --count N --thesis ... --strategy-version vX` — records a paper fill at the live take-side price with Kalshi's real fee. Hard guards in `agent/ledger.py` (not agent-tunable): $1,000 paper bankroll, ≤$50/trade, ≤25 open, no duplicate positions, weather markets only (category "Climate and Weather" or KXHIGH/KXLOW ticker prefix; operator mandate since 2026-07-13).
